...
async def register_post():
    ...
    if key_valid:
        already_used = await glob.db.fetch('SELECT used FROM beta_keys WHERE beta_key = %s', key)
        if int(already_used['used']):
  ...
  # use beta key
        await glob.db.execute('UPDATE beta_keys SET used = 1, user = %s WHERE beta_key = %s', [username, key])
    if glob.config.debug:
